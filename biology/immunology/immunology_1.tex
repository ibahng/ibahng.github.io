    ...
    \begin{core}{History: \textit{Edelman \& Porter}}
      ...
      \begin{enumerate}
        ...
        \item \textbf{Gerald Edelman} (\textit{Mercaptoethanol})
          ...
          Edelman employed mercaptoethanol to \textbf{reduce the disulfide bonds} that keep the light and heavy chains interconnected with one another, thereby enabling each chain to be isolated and studied individually.

          \myfig{0.75}{immunology_figures/edelman}{Antibody cleavage via mercaptoethanol; \textit{ph}}{fig:edelman} % ask gemini to remvoe the labels in the Ig domains

        \item \textbf{Rodney Porter} (\textit{Pepsin \& Papain})

          Porter demonstrated that pepsin cleaves preferentially at the \ul{\textbf{bottom of the hinge}} between the arms and the stem, thereby disconnecting the two individual heavy chain Fc sections from the top half of the molecule and yielding a single \ul{\textit{Fab2 fragment}}.

          \myfig{0.7}{immunology_figures/porter1}{Antibody cleavage via pepsin}{fig:porter1}

          By contrast, he found that papain cleaves preferentially at the \ul{\textbf{top of the hinge}} between the arms and the stem, disconnecting the Fc sections---which remain joined to one another by disulfide bonds---from two separate \textit{Fab} fragments.

          \myfig{0.65}{immunology_figures/porter2}{Antibody cleavage via papain}{fig:porter2}

      \end{enumerate}

      Notably, structurally divalent F(ab')2 fragments remain capable of precipitating or cross-linking antigens to form extended immune complexes, whereas monomeric Fab fragments lack this cross-linking ability.
